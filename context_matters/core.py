...
class FMDSim(rli.Simulation):
    ...
    def __init__(self, Agent, Environment, **kwargs):
        ...
        super(self.__class__, self).__init__(Agent, Environment, **kwargs)
        ...
        area_res = kwargs.pop('area_resolution', 1001)
    ...
    def full2obs(self, current_s):
        ...
        if (current_s.terminal):
            current_s.obs = (-9999, -9999)
        else:
            if not current_s.area:
                current_s.obs = (current_s.n_inf, 0)
            else:
                ind = np.digitize([current_s.area], self.abins, \
                    right = True)[0]
                current_s.obs = (current_s.n_inf, self.abins[ind])
        return current_s
    ...
    def collect_data(self, current_s, current_a, reward, next_s):
        ...
        if not(current_s.terminal):
            
            # define conditions for farm inclusion in outbreak area calculation
            cond1 = (current_s.status > (self.env.delay_latency)) # + self.env.delay_detect
            ...
            notified_vertices = self.env.coords[current_s.notified_ind]
        ...
        # Calculate summaries of the outbreak to save to a dataframe
        n_exp = np.sum(current_s.status > 0)
        n_inf = np.sum(current_s.status > self.env.delay_latency)
        current_s.n_inf = n_inf
        n_not = np.sum(current_s.status > (self.env.delay_detect + self.env.delay_latency))
        n_cull = np.sum(current_s.status == -1)
